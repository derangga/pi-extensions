...
- Every question also carries an automatically appended "Chat about this" row. When the user picks it, the dialog closes without that question being answered and the tool result says so explicitly: end your turn, then treat the user's next message as their clarification of that question. Questions answered before the pick stay answered; re-ask only the missing ones after the discussion.
