...
import {
  ...
  PREVIEW_MIN_WIDTH,
  previewSourceWidth,
} from "../src/view/components/preview/preview-layout-decider.js";
...
describe("crossTabPreviewBudget", () => {
  ...
  it("takes the max across all questions", () => {
    const qs = [
      question([{ label: "A", preview: "short" }]),
      question([{ label: "B", preview: "x".repeat(80) }]),
    ];
...
describe("crossTabLeftWidthWithDonation", () => {
  ...
  it("short labels still donate newly available width", () => {
    ...
    expect(crossTabLeftWidthWithDonation(tabs, itemsByTab, qs, 200)).toBe(
      Math.floor(200 * MAX_LEFT_RATIO),
    );
  });
  ...
  it("floors at MIN_LEFT even with donation", () => {
    ...
    const qs = [question([{ label: LONG_LABEL, preview: "tiny" }])];
    expect(crossTabLeftWidthWithDonation(tabs, itemsByTab, qs, 200)).toBeGreaterThanOrEqual(
      MIN_LEFT,
    );
  });
...
describe("decideLayout", () => {
  // Mutation testing found this untested: gating on terminal width ALONE left
  // every other test green. Both dimensions matter, because the dialog pane can
  // be narrower than the terminal that contains it.
  it("goes side by side only when terminal AND pane both clear the floor", () => {
    expect(decideLayout(PREVIEW_MIN_WIDTH, PREVIEW_MIN_WIDTH)).toBe("side-by-side");
  });

  it("stacks when the pane is narrow even though the terminal is wide", () => {
    expect(decideLayout(200, PREVIEW_MIN_WIDTH - 1)).toBe("stacked");
  });

  it("stacks when the terminal is narrow even though the pane claims to be wide", () => {
    expect(decideLayout(PREVIEW_MIN_WIDTH - 1, 200)).toBe("stacked");
  });

  it("stacks when both are below the floor", () => {
    expect(decideLayout(40, 40)).toBe("stacked");
  });

  it("treats the floor as inclusive on both dimensions", () => {
    expect(decideLayout(PREVIEW_MIN_WIDTH, 200)).toBe("side-by-side");
    expect(decideLayout(200, PREVIEW_MIN_WIDTH)).toBe("side-by-side");
    expect(decideLayout(PREVIEW_MIN_WIDTH - 1, PREVIEW_MIN_WIDTH)).toBe("stacked");
    expect(decideLayout(PREVIEW_MIN_WIDTH, PREVIEW_MIN_WIDTH - 1)).toBe("stacked");
  });
