...
} from "../src/state/selectors/derivations.js";
import { selectActiveView } from "../src/state/selectors/focus.js";
...
describe("selectConfirmedIndicator", () => {
  it("returns undefined when the question is multiSelect", () => {
    ...
    expect(
      selectConfirmedIndicator([q({ multiSelect: true })], 0, answers, itemsRegular),
    ).toBeUndefined();
  });
  ...
  it("returns the kind:'other' index + labelOverride when the prior answer was kind:'custom'", () => {
    ...
    expect(selectConfirmedIndicator([q()], 0, answers, itemsWithOther)).toEqual({
      index: 2,
      labelOverride: "Hello",
    });
  });
...
describe("selectConfirmedIndicator — kind matrix", () => {
  ...
  it("returns the kind:'other' index + labelOverride when prior was kind:'custom'", () => {
    ...
    expect(selectConfirmedIndicator(questions, 0, answers, items)).toEqual({
      index: 1,
      labelOverride: "Hello",
    });
  });
