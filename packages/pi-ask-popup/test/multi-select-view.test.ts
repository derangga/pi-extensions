...
import { lineAt, makeTheme, stripAnsi } from "./fixtures.js";
import { describe, expect, it } from "vitest";
import { makeMultiSelectViewProps as makeProps } from "./fixtures.js";
...
describe("MultiSelectView.render", () => {
  ...
  it("separates the checkbox from the label by exactly ONE space", () => {
    ...
    const raw = stripAnsi(lineAt(lines, 0));
  ...
  it("focused=false suppresses the active-row pointer (no doubled cursor)", () => {
    ...
    const rawFocused = focused.map((l) => l);
    expect(stripAnsi(lineAt(rawFocused, 1)).startsWith("❯ ")).toBe(true); // active pointer on selected row
    ...
    const rawRefocused = refocused.map((l) => l);
    expect(stripAnsi(lineAt(rawRefocused, 1)).startsWith("❯ ")).toBe(true);
  });
  ...
  it("active option uses ACTIVE_POINTER and accent styling", () => {
    ...
    expect(stripAnsi(lineAt(lines, 0)).startsWith("❯ ")).toBe(false); // inactive rows do not start with active pointer
  });
  ...
  it("row 1 inactive unchecked renders as '  1. [ ] LABEL'", () => {
    ...
    const q = question();
    const m = makeView(q, makeProps(q, { optionIndex: 1 }));
    const raw = stripAnsi(lineAt(m.render(80), 0));
  ...
  it("row 2 active checked renders as '❯ 2. [✔] LABEL'", () => {
    ...
    const raw = stripAnsi(lineAt(m.render(80), 1));
  ...
  it("setProps mutates props visible to next render (active row moves)", () => {
    ...
    const lines = m.render(80);
    expect(stripAnsi(lineAt(lines, 0)).startsWith("❯ ")).toBe(false);
    expect(lines[2]).toContain("❯ ");
  });
});
...
describe("MultiSelectView — 'Type something.' row", () => {
  ...
  it("keeps the draft visible on the other row while another option has focus", () => {
    ...
    const m = makeView(
      q,
      makeProps(q, { optionIndex: 0, inputMode: false, inputBuffer: "draft answer" }),
    );
  ...
  it("number column fits N+1 (the other row's number) without truncation", () => {
    ...
    const m = makeView(q, makeProps(q));
    const lines = m.render(80);
    const raw = stripAnsi(lineAt(lines, 9)); // other row
