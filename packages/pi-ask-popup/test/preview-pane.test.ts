...
import {
  ...
  PreviewPane,
  renderBorderedBox,
} from "../src/view/components/preview/preview-pane.js";
...
describe("PreviewPane — right-aligned preview with a bounded options column", () => {
  ...
  it("content-sized preview boxes share the right edge while retaining their natural widths", () => {
    ...
    const shortMD = lineAt(extractPreviewColumnLines(shortLines), 0).indexOf("MD[");
    ...
    const longMD = lineAt(extractPreviewColumnLines(longLines), 0).indexOf("MD[");
  ...
  it("side-by-side: adaptive left width adjusts options column based on label content", () => {
    ...
    const mdIdx = lineAt(preview, 0).indexOf("MD[");
...
describe("renderBorderedBox helper", () => {
  it("wraps lines in 4-sided border with `┌─┐│└┘` corners", () => {
    ...
    expect(lineAt(out, 1).startsWith("│")).toBe(true);
    expect(lineAt(out, 1).endsWith("│")).toBe(true);
  ...
  it("right-pads content lines so the right `│` lands at fixed column", () => {
    ...
    expect(visibleWidth(lineAt(out, 1))).toBe(20);
  });
...
describe("PreviewPane — notes affordance row (Slice 4 height-stable affordance)", () => {
  ...
  it("never lets the notes affordance overflow the render width, however narrow", () => {
    // Upstream drove this with a 65-column French locale string, proving a long
    // affordance slides left rather than clipping. Localization is gone, so
    // NOTES_AFFORDANCE_TEXT is a fixed 27 columns and can never exceed the
    // 44-column box floor; that input is now unreachable. The property it was
    // really protecting still is, from the other direction: squeeze the terminal
    // instead of stretching the string.
    for (const width of [30, 40, 50, 60, 80, 120]) {
      const { pane, optionListView } = makePane(question, () => width);
      optionListView.setProps({ selectedIndex: 0, focused: true, inputBuffer: "" });
      pane.setProps({ notesVisible: false, selectedIndex: 0, focused: true, inputMode: false });
...
describe("PreviewPane — adaptive left column width", () => {
  ...
  it("cross-tab max keeps width stable — same getter produces same leftWidth for different items", () => {
    ...
    const getter = (w: number) =>
      Math.max(adaptiveLeftWidth(shortItems, 2, w), adaptiveLeftWidth(longItems, 2, w));
