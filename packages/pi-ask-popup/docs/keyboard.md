...
Confirming `Chat About This` produces no answer at all: the dialog closes and the result carries a `chatRequested` marker for that question instead. See [Tool schema](./tool-schema.md).
